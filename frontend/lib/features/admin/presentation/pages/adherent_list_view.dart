  ...
  Future<void> _fetchAdherents() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          _adherents = data.map((json) => AuthUser(
      ...
      child: Column(
        ...
        children: [
          ...
          if (_isLoading)
            ...
            const Expanded(child: Center(child: Text("Aucun adhérent trouvé.", style: TextStyle(color: AppColors.textGray))))
          else
            Expanded(
              child: Container(
                ...
                child: ClipRRect(
                  ...
                  child: SingleChildScrollView(
                    child: DataTable(
                      ...
                      rows: _adherents.map((adherent) => DataRow(
                        cells: [
                          DataCell(Text(adherent.displayName ?? '-', style: const TextStyle(color: AppColors.textWhite))),
                          DataCell(Text(adherent.email, style: const TextStyle(color: AppColors.textGray))),
                          DataCell(Container(
                            ...
                            child: const Text('Adhérent', style: TextStyle(color: AppColors.accentBlue, fontSize: 12)),
                          )),
                          DataCell(Row(
                            children: [
                              IconButton(
